...
class HadithSearchFragment : Fragment()
{
    ...
    private lateinit var svHadiths: SearchView
    ...
    ): View?
    {
        ...
        val view = inflater.inflate(R.layout.fragment_hadith_search, container, false)
        ibFilter = view.findViewById(R.id.ibFilter)
        tvResults = view.findViewById(R.id.tvResults)
        svHadiths = view.findViewById(R.id.svHadiths)
        rvHadiths = view.findViewById(R.id.rvHadiths)

        if (hadiths.isNotEmpty()) tvResults.visibility = View.GONE
        ...
        ibFilter.setOnClickListener {
            val fragment = HadithFilterFragment.newInstance()
            fragment.show(activity!!.supportFragmentManager, "filter-fragment")
        }

        svHadiths.setOnQueryTextListener(object : SearchView.OnQueryTextListener
        {
            override fun onQueryTextSubmit(query: String): Boolean
            {
                viewModel.getHadiths(
                    hadithEnglish = query,
                    book = viewModel.bookPair.value!!.second,
                    status = viewModel.statusPair.value!!.second
                )
                return false
            }
    ...
    override fun onDestroy()
    {
        super.onDestroy()
        tts.shutdown()
        viewModel.hadiths.postValue(Resource.Success(Hadith()))
        viewModel.statusPair.postValue(Pair(R.id.rbStatusAll, ""))
        viewModel.bookPair.postValue(Pair(R.id.rbBooksAll, ""))
    }
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder
        {
            val view =
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.row_hadith_search, parent, false)
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int)
        {
            holder.apply {
                hadiths[position].let { hadith ->
                    ...
                    tvBookChapter.text = "${hadith.book.bookName}, ${hadith.chapter.chapterEnglish}"
                    tvNarration.text = hadith.englishNarrator

                    val searchQuery = svHadiths.query.trim().toString()
                    val hadithEnglish = hadith.hadithEnglish.replace(
                        searchQuery,
                        "<span style='background-color:yellow'>$searchQuery</span>",
                        true
                    )
                    tvHadith.text = Html.fromHtml(hadithEnglish, Html.FROM_HTML_MODE_LEGACY);
    ...
    {
        @JvmStatic
        fun newInstance() = HadithSearchFragment()
    }
}